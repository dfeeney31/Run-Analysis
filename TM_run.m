...
data = importForcesTM('C:\Users\Daniel.Feeney\Dropbox (Boa)\Treadmill TMM\Data\WalkTest_9 - Report1.txt');
...
filt_forceZ = filtfilt(b,a,data.ForceZ(1:9999));
filt_forceY = -1 .* filtfilt(b,a,data.ForceY(1:9999)); %This will be used for the M/L and A/P forces
filt_forceX = filtfilt(b,a,data.ForceX(1:9999)); %This will be used for the M/L and A/P forces
...
APforce = zeros(floor(length(pks)/2)/2,301);
MLforce = zeros(floor(length(pks)/2)/2,301);
for peak = 2:2:length(pks)
    tmp_location = locs(peak);
    vertForce(peak/2,:) = filt_forceZ(tmp_location - 150 : tmp_location +150);   
    APforce(peak/2,:) = filt_forceY(tmp_location - 150 : tmp_location +150); 
    MLforce(peak/2,:) = filt_forceX(tmp_location - 150 : tmp_location +150); 
end
...
APforce2 = zeros(floor(length(pks)/2)/2,301);
MLforce2 = zeros(floor(length(pks)/2)/2,301);
